...
def extract_data():
    ...
    filtered_endpoint = f"{API_ENDPOINT}?$filter=date_trans ge {date_threshold}"
    response = requests.get(filtered_endpoint, headers=headers)
...
def load_to_bigquery(**context):
    ...
    date_threshold = (datetime.utcnow() - timedelta(days=30)).strftime('%Y-%m-%d')
    delete_query = f"DELETE FROM `{BQ_DATASET_TABLE}` WHERE date_trans >= '{date_threshold}'"
